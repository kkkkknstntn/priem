...
class Abitura:
    def __init__(self, row, prior, osoboe, tables,  dop):
        self.row = row
        self.prior = prior
        self.osoboe = osoboe
        self.tables = tables
        ...
        for row in table.find_all('tr'):
            ...
            if len(cols) > 0:
                if cols[1] not in abituras:
                    ...
                    abituras[cols[1]] = Abitura(cols[1:8], prior, osoboe, tables, dop)
    ...
    for file in abituras[ab].tables:
        ...
        if ab not in abituras_old[file]:
            ...
            new_row = row[0:1] + ['', ''] + abituras[ab].prior[0:5] + [
                ' '.join(abituras[ab].prior[5:])] + row[1:] + [' '.join(abituras[ab].dop), '', 'госуслуги', '']
